...
class VoiceScout(Node):
   ...
   def status_screen_cb(self):
       ...
       # Visual Sync: Display LISTENING when microphone is actively listening
   ...
   def watchdog_logic(self):
       ...
       if self.front_obstacle_detected:
           ...
           if not self.stopped_for_obstacle:
               self.voice.beep(2)
               # Obstacle Replanning: Only trigger if we are actively guiding
   ...
   def trigger_arrival_prompt(self):
       ...
       if 'y' in ans or 'never' in ans or is_cancel_query(ans):
           ...
           # If user says "no", we just try the next known location in memory.
           # (We only delete from memory in wander mode if a scan proves it's gone)
           self.target_index += 1
           self.start_guidance()
   ...
   def rgb_cb(self, msg):
       ...
       if self.state == "WANDERING":
           ...
           # Clean Memory: If we are looking at a known location but nothing is detected there, remove it
           robot_pos = self.get_robot_pose()
           if robot_pos:
               for item_type, locations in self.object_memory.items():
                   for i, loc in enumerate(locations[:]):
                       # Only check locations close to the robot (within vision range)
                       dist = math.hypot(loc[0] - robot_pos[0], loc[1] - robot_pos[1])
                       if dist < 2.5:
                           # Is there a detection of this item type currently near this coordinate?
                           match_found = any(d[0] == item_type and math.hypot(d[1][0]-loc[0], d[1][1]-loc[1]) < 0.6 
                                             for d in current_frame_detections)
                           if not match_found:
                               self.get_logger().info(f"Memory Cleanup: {item_type} at {loc} no longer visible. Deleting.")
